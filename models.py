...
class LorentzInvariantFeatures(nn.Module):
    ...
    def forward(self, four_momenta):
        ...
        mass_squared = four_momenta[..., 3]**2 - (four_momenta[..., 0]**2 + 
                                                 four_momenta[..., 1]**2 + 
                                                 four_momenta[..., 2]**2)
        mass = torch.where(mass_squared>=0, mass_squared.sqrt(), -((-mass_squared).sqrt()))
...
class TestNetwork(nn.Module):
        def __init__(self, use_lorentz_invariant_features=True, bottleneck_attention=None, feature_set=['pt', 'eta', 'phi', 'm', 'tag'], num_classes=3, hidden_dim=256, num_heads=4, dropout_p=0.0, embedding_size=32, num_attention_blocks=3, include_mlp=True, hidden_dim_mlp=None, hidden_dim_attn=None, num_particle_types=5):
            super().__init__()
            ...
            self.attention_blocks = nn.ModuleList([
                nn.ModuleDict({
                    ...
                    # 'input_projection': nn.Linear(hidden_dim, hidden_dim_attn),
                    **({f'input_projection': nn.Linear(hidden_dim, self.hidden_dim_attn)} if (self.hidden_dim_attn is not None) else {}),
                    ...
                    **({f'bottleneck_up': nn.ModuleList([
                        ...
                        for _ in range(num_heads)])} if (self.bottleneck_attention is not None) else {}),
                    # 'output_projection': nn.Linear(hidden_dim_attn, hidden_dim),
                    **({f'output_projection': nn.Linear(self.hidden_dim_attn, hidden_dim)} if (self.hidden_dim_attn is not None) else {}),
...
class GNNParticleClassifier(nn.Module):
    def __init__(
        self,
        num_particle_types=6,     # include 'None' padding
        embedding_size=32,
        feature_set=['pt','eta','phi','m','tag'],
        hidden_dim=128,
        num_classes=3,
        k=8,                      # for k-NN graph
        num_heads=4,
        conv_type='edge',         # 'edge' or 'gat'
        num_layers=3,
        aggr='mean',              # for EdgeConv
    ):
        super().__init__()
        self.k = k
        self.conv_type = conv_type

        # 1) per-object feature prep
        self.type_emb = nn.Embedding(num_particle_types, embedding_size)
        self.lorentz = LorentzInvariantFeatures(feature_set=feature_set[:4])
        in_feat_dim = len(feature_set) - 1  # skip type in feature_set
        self.pre_mlp = nn.Sequential(
            nn.Linear(in_feat_dim + embedding_size, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, hidden_dim),
        )
        ...
        for _ in range(num_layers):
            if conv_type == 'edge':
                # EdgeConv with a small MLP for messages
                mlp = nn.Sequential(
                ...
                self.convs.append(
                    GATConv(hidden_dim, hidden_dim // 4, heads=num_heads, concat=True)
                )
            else:
                raise ValueError("conv_type must be 'edge' or 'gat'")
    ...
    def forward(self, object_features, object_types):
        # object_features: [B, N, V], where V includes: [4-mom, other...]
        B, N, V = object_features.size()
        # 1) split out
        object_types = object_types.long()           # [B,N]
        four_mom = object_features[...,:4]             # [B,N,4]
        extras = object_features[...,4:]                # [B,N, V-5]

        # 2) build node features
        inv = self.lorentz(four_mom)                # [B,N, len(set)=4]
        t_emb = self.type_emb(object_types)                # [B,N, emb]
        x = torch.cat([inv, extras, t_emb], dim=-1) # [B,N, D_in]
        x = self.pre_mlp(x)                         # [B,N, hidden_dim]

        # 3) flatten for PyG: nodes = B*N, batch index vector
        x = x.view(B*N, -1)
        batch = torch.arange(B, device=x.device).repeat_interleave(N)  # [B*N]
        # mask out padding nodes: assume type == num_particle_types-1 means 'None'
        valid = (object_types.view(-1) != (self.type_emb.num_embeddings-1))

        # 4) build graph edges: either fully connected or k-NN
        edge_index = knn_graph(x, k=self.k, batch=batch, loop=True)  # [2, E]
        ...
        # 6) global pooling (mean over each graph, ignoring padding)
        # mask invalid nodes by zeroing their features
...
class TestNetworkClassifier(nn.Module):
        def __init__(self, use_lorentz_invariant_features=True, bottleneck_attention=None, feature_set=['pt', 'eta', 'phi', 'm', 'tag'], num_classes=3, hidden_dim=256, num_heads=4, dropout_p=0.0, embedding_size=32, num_attention_blocks=3, include_mlp=True, hidden_dim_mlp=None, hidden_dim_attn=None, num_particle_types=5):
            super().__init__()
            self.bottleneck_attention = bottleneck_attention
            self.num_attention_blocks = num_attention_blocks
...
class TestNetworkClassifierImmediatePool(nn.Module):
        def __init__(self, use_lorentz_invariant_features=True, bottleneck_attention=None, feature_set=['pt', 'eta', 'phi', 'm', 'tag'], num_classes=3, hidden_dim=256, num_heads=4, dropout_p=0.0, embedding_size=32, num_attention_blocks=3, include_mlp=True, hidden_dim_mlp=None, hidden_dim_attn=None, num_particle_types=5):
            super().__init__()
            self.bottleneck_attention = bottleneck_attention
            self.num_attention_blocks = num_attention_blocks
